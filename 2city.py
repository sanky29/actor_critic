        ...
        if (action[0] == self.tp or action[1] > self.tl or action[1] > self.demand[action[0]]):
            ...
            if (self.td == 0):
                ...
                if (self.tl == 0.0):
                    ...
                    self.tp = 0
            self.state = np.power(np.sum(np.multiply(self.demand,self.distance[self.tp]))*(2 + self.tp),self.tl)
            return ([self.state] ,-1*self.distance[z][int (action[0])],a)
...
class actor_critic:
    ...
    def __init__(self):
        ...
        '''the actor is here'''
        self.l5 = keras.layers.Dense(10, activation='softmax')(self.l1)
        ...
        self.inputq1 = keras.Input(shape=(2,))
        self.inputq = keras.Input(shape=(1,))
        self.inp = keras.layers.Concatenate(axis=-1)([self.inputq1, self.inputq])
        self.l2q = keras.layers.Dense(5, activation='linear', use_bias=True)(self.inp)
        ...
        # self.l14q = keras.layers.Dense(7, activation='tanh', use_bias=True, bias_initializer=keras.initializers.Constant(-1))(self.l13q)
        # self.l15q = keras.layers.Dense(3, activation='tanh', use_bias=True, bias_initializer=keras.initializers.Constant(2))(self.l14q)
        self.l16q = keras.layers.Dense(1,activation= 'linear' ,use_bias = True, dtype= tf.float64)(self.l2q)
        ...
        self.criticq.compile(optimizer= 'RMSprop' , loss= 'mean_squared_error')
...
class RLagent:
    ...
    def train1(self, e):
        ...
        for i in range(0, len(episode) - 2):
            self.agent.next = tf.identity(self.agent.criticq.predict([np.array([episode[i+1][0]]),np.array([[episode[i+2][2]]])]), dtype= tf.float64)
            ...
            self.agent.criticq.fit(np.array([[np.array([episode[i][0]]), np.array([[episode[i + 1][2]]])]]), np.array([self.agent.next + self.agent.reward]), batch_size= None)
            z = s.run(tf.square((self.agent.reward) + (self.agent.next) - self.agent.criticq.predict([np.array([episode[i][0]]),np.array([[episode[i+1][2]]])])))
            print(z)
        ...
        self.agent.criticq.fit(np.array([[np.array([episode[len(episode) - 2][0]]), np.array([[episode[len(episode) - 1][2]]])]]),np.array([self.agent.next + self.agent.reward]), batch_size=None)
        z = s.run(tf.square((self.agent.reward) +(self.agent.next) - self.agent.criticq.predict([np.array([episode[len(episode) - 2][0]]), np.array([[episode[len(episode) - 1][2]]])])))
        print(z)
